...
export class FetchFeedbackRepository implements FeedbackRepository {
    async send(feedback: Feedback, endpoint: string, apiKey: string): Promise<void> {
        const maxRetries = 3;
        let attempt = 0;
            ...
            try {
                const response = await fetch(endpoint, {
                    ...
                    headers: {
                        ...
                        "minders-api-key": "minders_secret_key",
                ...
                if (response.status < 500) throw new Error(`CLIENT_ERROR:${response.status}`);
